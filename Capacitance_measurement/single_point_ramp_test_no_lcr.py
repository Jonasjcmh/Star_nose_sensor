"""
single_point_ramp_test_no_lcr.py — Star-Nose Sensor | Fixed-Ramp Single-Point Test (no LCR)
==========================================================================================
Identical to single_point_ramp_test.py, but WITHOUT the LCR-6100 capacitance
meter. Use this when the LCR is not connected and you only want the robot motion
+ load-cell / F-T characterisation of a fixed-ramp single-point press.
...
------------------------
  ...
  indentation rate (the cruise speed) — better for force characterisation.
...
------------
  ...
  5. After the cycle, plots are generated automatically.
  6. Press depth is measured downward from the calibrated contact surface.

Signals logged (no capacitance column — LCR not used):
  • UR5 TCP position and F/T sensor
  • FUTEK load cell (AI0)

Usage
-----
  python single_point_ramp_test_no_lcr.py
  python single_point_ramp_test_no_lcr.py --point 10 --depth 9 --ramp 2 --locate 3 --post 8
  python single_point_ramp_test_no_lcr.py --point 5 --depth 3 --ramp 1            # hold=5
  python single_point_ramp_test_no_lcr.py --point 10 --depth 2 --ramp 2 --no-plot
"""
...
def _phase_tail_mean(phase, window_s=SETTLE_WINDOW_S, rate_hz=100):
    """Average load-cell force (N) over the LAST `window_s` of the most recent
    run of `phase`. Reporting this settled mean — instead of one instantaneous
    sample — cuts sensor/electrical noise and is far more repeatable.
    Returns (load_N_mean, n_samples_used)."""
    ...
    return ld_mean, len(rows)

def _move_logged(rtde_c, target, vel, accel, pt, depth_mm, phase, rate_hz):
...
def save_dataset(pt):
    ...
    path = os.path.join(LOG_DIR, f'single_point_ramp_noLCR_P{pt:02d}_{ts}.csv')
...
def do_indentation(rtde_c, pt, depth_mm, locate_s, hold_s, post_s, ramp_s, rate_hz=100):
    """
    ...
    _log_row(pt, depth_mm, 'locate')
    _log_timed(pt, depth_mm, 'locate', rate_hz, locate_s)
    lc_m, n = _phase_tail_mean('locate', SETTLE_WINDOW_S, rate_hz)
    print(f'     [locate]   LC={lc_m:.2f} N   (mean of last {n} samples)')

    # ── Press (ramp_s) ─────────────────────────────────────────────────────────
    print(f'     → press   (depth {depth_mm:.2f} mm, target ramp {ramp_s:.1f}s) ...')
    t_press = _move_logged(rtde_c, pressed, v_press, a_press,
                           pt, depth_mm, 'press', rate_hz)
    ...
    lc_m, n = _phase_tail_mean('hold', SETTLE_WINDOW_S, rate_hz)
    print(f'     [hold]     LC={lc_m:.2f} N   (mean of last {n} samples)')

    # ── Retract (ramp_s) ───────────────────────────────────────────────────────
    print(f'     → retract  (back to surface, target ramp {ramp_s:.1f}s) ...')
    t_ret = _move_logged(rtde_c, surface, v_press, a_press,
                         pt, depth_mm, 'retract', rate_hz)
    print(f'     [retract]  target ramp = {ramp_s:.1f}s   actual = {t_ret:.2f}s')

    # ── Post ─────────────────────────────────────────────────────────────────
    _log_timed(pt, depth_mm, 'post', rate_hz, post_s)
    lc_m, n = _phase_tail_mean('post', SETTLE_WINDOW_S, rate_hz)
    print(f'     [post]     LC={lc_m:.2f} N   (mean of last {n} samples)')
...
def plot_results(rows, pt, depth_mm, locate_s, hold_s, post_s, show=True):
    ...
    fig.suptitle(f'Single-point ramp test (no LCR) — P{pt:02d}   depth={depth_mm:.2f} mm   '
    ...
    # (1,0) force vs depth (loading curve)
    ax = axs[1, 0]
    sc = ax.scatter(depth, load, c=t, cmap='viridis', s=10)
    ax.set_xlabel('indentation depth (mm, down +)'); ax.set_ylabel('load cell (N)')
    ax.set_title('Force vs depth'); ax.grid(alpha=0.3)
    cb = fig.colorbar(sc, ax=ax); cb.set_label('time (s)')

    # (1,1) Fz (UR) vs time
    ax = axs[1, 1]; shade(ax)
    ax.plot(t, fz, color='#9467bd', lw=1.2)
    ax.set_xlabel('time (s)'); ax.set_ylabel('Fz UR (N)')
    ax.set_title('UR F/T Fz vs time'); ax.grid(alpha=0.3)
    ...
    ts   = datetime.now().strftime('%Y%m%d_%H%M%S')
    path = os.path.join(PLOT_DIR, f'single_point_ramp_noLCR_P{pt:02d}_{ts}.png')
...
def plot_overlay(rows, pt, depth_mm, locate_s, hold_s, post_s, show=True):
    """
    Single time-axis figure overlaying depth and force, each on its own y-axis,
    with the background shaded by phase (locate / press / hold / retract / post).
    """
    import matplotlib
    if not show:
        matplotlib.use('Agg')      # headless / save-only
    ...
    C_DEPTH, C_FORCE = '#2ca02c', '#d62728'

    fig, ax_d = plt.subplots(figsize=(13, 6))
    fig.suptitle(f'Single-point ramp overlay (no LCR) — P{pt:02d}   depth={depth_mm:.2f} mm   '
                 f'dwell: locate={locate_s:.1f}s  hold={hold_s:.1f}s  post={post_s:.1f}s',
                 fontsize=13, fontweight='bold')
    ...
    os.makedirs(PLOT_DIR, exist_ok=True)
    ts   = datetime.now().strftime('%Y%m%d_%H%M%S')
    path = os.path.join(PLOT_DIR, f'single_point_ramp_noLCR_overlay_P{pt:02d}_{ts}.png')
    fig.savefig(path, dpi=130, bbox_inches='tight')
...
def parse_args():
    p = argparse.ArgumentParser(
        description='Fixed-ramp single-point indentation test with auto-plotting (no LCR)',
...
def main():
    ...
    print('=' * 65)
    print('  Fixed-Ramp Single-Point Indentation Test (no LCR) — Star-Nose Sensor')
    print('=' * 65)
    ...
    # ── Ready prompt ──────────────────────────────────────────────────────────
    print(f'\n  ┌─────────────────────────────────────────────────────┐')
    print(f'  │  Ready to press point  P{pt:02d}  (no LCR connected)     │')
    print(f'  └─────────────────────────────────────────────────────┘')

    path = None
    try:
        input('\n  Press ENTER when ready ... ')
        do_indentation(rtde_c, pt, depth_mm,
