"""
collect.negz_noloadcell.py — collect UR5 fz data for ONE weight in the
NEGATIVE z axis, using the UR robot ONLY (no FUTEK load cell).

Same flow as collect.negz.py, but it does NOT depend on the FUTEK load
cell: ai0 is still logged (to keep the same CSV schema) but is ignored.
The tool is expected to already be sitting in the -z pose — this script
does NOT move the wrist at all. It only checks the tool is vertical
within tolerance and aborts if it is off.

One run = one weight = one CSV. Run it again for the next weight.

Usage:
  python collect.negz_noloadcell.py
      -> prompts: "Enter weight in grams:"

  # or pass the weight directly, skipping the prompt:
  python collect.negz_noloadcell.py --weight 200
"""
...
def collect_one(args):
    ...
    # ── Perpendicularity check (actual live pose) — DO NOT move the wrist ──
    ...
    if tilt > args.tilt_tol_deg:
        ...
        sys.exit(
            ...
            f"exceeds {args.tilt_tol_deg:.2f} deg tolerance. Move the tool to "
            "a vertical -z pose, then re-run."
        )
    ...
    try:
        input("\n[collect] Tool mounted (-z pose), NO weight applied. "
    ...
    csv_path = os.path.join(
        LOG_DIR, f"fzcal_{args.tip}_negz_{weight:g}g_{ts}.csv")
