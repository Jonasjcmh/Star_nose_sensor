"""
capacitance_ramp_collector.py — Star-Nose Sensor | Multi-Point Ramp Collector
=============================================================================
Combines the multi-point workflow of capacitance_dataset_collector.py with the
fixed-ramp trapezoidal press and tail-averaging of single_point_ramp_test.py.

This script ONLY collects and saves data. Plotting/analysis is done separately
by analyze_ramp_collector.py (which reads the saved CSV).

What it does
------------
  • You enter ALL parameters once at the start, applied to every point:
        samples/point · depth · ramp(press&retract) · hold · locate · post
  • Runs N "samples" (rounds). Each round visits all 19 points ONCE in a
    random order (every point measured exactly once per round).
  • Each indentation: locate → press → hold → retract → post, with the
    press/retract speed & accel derived so each move takes exactly `ramp`
    seconds regardless of depth (trapezoidal profile).
  • Like the original collector, it PAUSES at each point so you can re-wire the
    LCR-6100 probes to that pad (press ENTER to continue).
...
Usage
-----
  python capacitance_ramp_collector.py
  python capacitance_ramp_collector.py --samples 5 --depth 2 --ramp 2 \
         --hold 5 --locate 5 --post 5
  # then plot:
  python analyze_ramp_collector.py            # newest CSV in logs/
"""
...
def _log_timed(pt, depth_mm, phase, round_idx, sample_idx, lcr, rate_hz, duration_s):
...
def _phase_tail_mean(phase, window_s=SETTLE_WINDOW_S, rate_hz=100):
    ...
    recent run of `phase` (the current point, since points are done in series).
    Settled mean instead of one sample → ~√N less noise. Returns
...
def _move_logged(rtde_c, target, vel, accel, pt, depth_mm, phase,
                 round_idx, sample_idx, lcr, rate_hz):
...
def save_dataset():
    ...
    path = os.path.join(LOG_DIR, f'ramp_collector_{ts}.csv')
...
def generate_plan(n_samples, seed=None):
    """N rounds; each round visits all 19 points once in a random order.
    Returns list of (round_idx, sample_idx, point)."""
...
def do_indentation(rtde_c, pt, depth_mm, round_idx, sample_idx, lcr,
    ...
    t_press = _move_logged(rtde_c, pressed, v_press, a_press,
                           pt, depth_mm, 'press', round_idx, sample_idx, lcr, rate_hz)
    print(f'     [press]    target ramp = {ramp_s:.1f}s   actual = {t_press:.2f}s')

    # ── Hold ──────────────────────────────────────────────────────────────────
    _log_timed(pt, depth_mm, 'hold', round_idx, sample_idx, lcr, rate_hz, hold_s)
    ...
    t_ret = _move_logged(rtde_c, surface, v_press, a_press,
                         pt, depth_mm, 'retract', round_idx, sample_idx, lcr, rate_hz)
    print(f'     [retract]  target ramp = {ramp_s:.1f}s   actual = {t_ret:.2f}s')

    # ── Post ─────────────────────────────────────────────────────────────────
    _log_timed(pt, depth_mm, 'post', round_idx, sample_idx, lcr, rate_hz, post_s)
...
def parse_args():
    p = argparse.ArgumentParser(
        description='Multi-point fixed-ramp capacitance collector (data only)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=__doc__)
    p.add_argument('--samples', type=int,   default=None, help='Samples (rounds) per point [ask, default 5]')
    p.add_argument('--depth',   type=float, default=None, help='Press depth mm [ask, default 2]')
...
def main():
    ...
    print('=' * 65)
    print('  Multi-Point Fixed-Ramp Capacitance Collector — Star-Nose Sensor')
    print('=' * 65)

    n_samples = args.samples if args.samples is not None else _ask_int(
        '\n  Samples per point (rounds) [5] > ', 5, 1, 100)
    ...
    print(f'\n  Samples per point : {n_samples}  (→ {n_samples * 19} indentations)')
    print(f'  Press depth       : {depth_mm:.2f} mm')
    print(f'  Ramp (press/retr) : {ramp_s:.1f} s each   '
          f'(cruise={v_press*1000:.3f} mm/s  accel={a_press*1000:.3f} mm/s²)')
    ...
    print(f'  ~Cycle time       : {per_cycle_s:.1f} s/point (excl. wiring & travel)')
    ...
    try:
        for step, (round_idx, sample_idx, pt) in enumerate(plan):
            ...
            try:
                do_indentation(rtde_c, pt, depth_mm, round_idx, sample_idx, lcr,
            ...
            # End of a round → all 19 points done; user can plot this round
            if step == total - 1 or plan[step + 1][0] != round_idx:
                print(f'\n  ✓ Round {round_idx + 1} complete (all 19 points). '
                      f'Plot later: python analyze_ramp_collector.py')
    except KeyboardInterrupt:
        print('\n  Interrupted')
    finally:
        _ft_stop.set()
        ...
        if path:
            print(f'  Data: {path}')
            print(f'  Plot it with:  python analyze_ramp_collector.py "{path}"')
    print('[done]')
