"""
single_point_test.py — Star-Nose Sensor | Single-Point Indentation Test
========================================================================
A lightweight, single-point version of capacitance_dataset_collector.py for
quick checks and tuning. You pick ONE point, specify the dwell time and the
press depth, the robot does ONE indentation, and the moment it finishes the
results are plotted automatically.

What it does
------------
  1. Press ONE chosen point once (locate → press → hold → retract → post).
  2. You SPECIFY each dwell time separately: --locate, --hold, --post (seconds).
     The hold dwell is forced to exceed the press/retract ramp time.
  3. After the cycle, plots are generated automatically:
        • Cp (pF) vs time          • load cell / Fz vs time
        • indentation depth vs time • Cp vs load (force–capacitance)
     Plots are saved to plots/ and shown if a display is available.
  4. Press depth is measured downward from the calibrated contact surface.
...
-----
  ...
  python single_point_test.py --point 10 --depth 2.0 --locate 3 --hold 5 --post 8
  python single_point_test.py --point 5 --depth 3 --hold 8          # locate/post asked
  python single_point_test.py --point 10 --depth 2 --hold 5 --no-plot
"""
...
# ── Ramp-time model (for reference / sanity warning) ───────────────────────────
...
def save_dataset(pt):
    ...
    path = os.path.join(LOG_DIR, f'single_point_P{pt:02d}_{ts}.csv')
...
def do_indentation(rtde_c, pt, depth_mm, lcr, locate_s, hold_s, post_s, rate_hz=100):
    """
    One step-impulse indentation, logging at rate_hz:
      locate (locate_s) → press → hold (hold_s) → retract → post (post_s)
    Each dwell has its own duration; press/retract are moves (not held).
    """
    ...
    st = get_robot_state(); Cp, _, _ = lcr.get_latest()
    print(f'     [locate]   LC={_ai0_to_n(st["ai0"]):.2f} N   Cp={Cp*1e12:.2f} pF')

    # ── Press ────────────────────────────────────────────────────────────────
    print(f'     → press   (depth {depth_mm:.2f} mm) ...')
    t_press = _move_logged(rtde_c, pressed, VEL_PRESS, ACCEL,
                           pt, depth_mm, 'press', lcr, rate_hz)
    print(f'     [press]    actual ramp = {t_press:.2f} s')

    # ── Hold (the user-specified dwell time) ──────────────────────────────────
    _log_timed(pt, depth_mm, 'hold', lcr, rate_hz, hold_s)
    st = get_robot_state(); Cp, _, _ = lcr.get_latest()
    print(f'     [hold]     LC={_ai0_to_n(st["ai0"]):.2f} N   Cp={Cp*1e12:.2f} pF')

    # ── Retract ──────────────────────────────────────────────────────────────
    print('     → retract  (back to surface) ...')
    _move_logged(rtde_c, surface, VEL_PRESS, ACCEL,
                 pt, depth_mm, 'retract', lcr, rate_hz)

    # ── Post ─────────────────────────────────────────────────────────────────
    _log_timed(pt, depth_mm, 'post', lcr, rate_hz, post_s)
    st = get_robot_state(); Cp, _, _ = lcr.get_latest()
    print(f'     [post]     LC={_ai0_to_n(st["ai0"]):.2f} N   Cp={Cp*1e12:.2f} pF')
...
def plot_results(rows, pt, depth_mm, locate_s, hold_s, post_s, show=True):
    ...
    fig.suptitle(f'Single-point test — P{pt:02d}   depth={depth_mm:.2f} mm   '
    ...
    ts   = datetime.now().strftime('%Y%m%d_%H%M%S')
    path = os.path.join(PLOT_DIR, f'single_point_P{pt:02d}_{ts}.png')
...
def plot_overlay(rows, pt, depth_mm, locate_s, hold_s, post_s, show=True):
    ...
    fig.suptitle(f'Single-point overlay — P{pt:02d}   depth={depth_mm:.2f} mm   '
                 f'dwell: locate={locate_s:.1f}s  hold={hold_s:.1f}s  post={post_s:.1f}s',
                 fontsize=13, fontweight='bold')
    ...
    os.makedirs(PLOT_DIR, exist_ok=True)
    ts   = datetime.now().strftime('%Y%m%d_%H%M%S')
    path = os.path.join(PLOT_DIR, f'single_point_overlay_P{pt:02d}_{ts}.png')
    fig.savefig(path, dpi=130, bbox_inches='tight')
...
def parse_args():
    p = argparse.ArgumentParser(
        description='Single-point indentation test with auto-plotting',
    ...
    p.add_argument('--hold',     type=float, default=None,
                   help='Dwell time (s) at full depth — must exceed the press/retract ramp (default: ask)')
...
def main():
    ...
    print('=' * 65)
    print('  Single-Point Indentation Test — Star-Nose Sensor')
    print('=' * 65)
    ...
    # The hold dwell must outlast the press AND retract ramps so the loaded
    # measurement is taken with the robot fully settled, not mid-motion.
    min_hold = ramp_s * 1.2
    ...
    hold_s = args.hold if args.hold is not None else _ask_float(
        f'  Hold dwell (s, at full depth; >{min_hold:.2f}s) [{max(5.0, min_hold):.1f}] > ',
        max(5.0, min_hold), 0.1, 120.0)
    ...
    # ── Enforce hold > ramp ───────────────────────────────────────────────────
    if hold_s < min_hold:
        print(f'  [warn] Hold ({hold_s:.2f}s) does not exceed the press/retract '
              f'ramp ({ramp_s:.2f}s) — raising hold to {min_hold:.2f}s.')
        hold_s = min_hold
    ...
    print(f'  Press/retract ramp: {ramp_s:.2f} s each (model)  →  hold is '
          f'{hold_s / ramp_s:.1f}× the ramp' if ramp_s > 0 else
          f'  Press/retract ramp: {ramp_s:.2f} s each (model)')
    ...
    try:
        ...
        do_indentation(rtde_c, pt, depth_mm, lcr,
                       locate_s=locate_s, hold_s=hold_s, post_s=post_s, rate_hz=rate_hz)
    except (EOFError, KeyboardInterrupt):
        print('\n  Interrupted.')
    except Exception as e:
        print(f'\n  [error] {e}')
