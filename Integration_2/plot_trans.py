...
def main():
    ...
    # ── 19 unit hexagons, centered on the RIGID-TRANSFORMED points but drawn ───
    # STRAIGHT (unrotated) rather than tilted with the fitted rotation — purely
    # a visual/reference grid choice, not meant to represent the sensor's true
    # taxel orientation.
    ...
    ax.set_title(
        f"Calibration points (rigid: rotation {rotation_deg:+.2f}° + translation)  "
        f"({total} total)",
