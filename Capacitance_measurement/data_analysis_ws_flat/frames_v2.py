...
class TouchSensorAnalyzer:
    ...
    def __init__(self, df: pd.DataFrame, point: int, depth_mm: float = None):
        ...
        if not self.rounds:
            # (df['point'] == point) & (df['depth_mm'] == depth_mm) matched zero rows.
            # Fail here with the actual available values instead of letting an empty
            # analyzer silently propagate into plot_combined_grid, where it shows up
            # as a cryptic "Number of columns must be a positive integer, not 0".
            available_points = sorted(df['point'].unique().tolist())
            msg = f"No rows matched point={point}"
            if depth_mm is not None:
                msg += f", depth_mm={depth_mm}"
                if 'depth_mm' in df.columns:
                    available_depths = sorted(df['depth_mm'].unique().tolist())
                    msg += f". File actually has depth_mm values={available_depths}"
            msg += f". File actually has points={available_points}."
...
if __name__ == '__main__':
    ...
    csv_path = '/home/divuthejo/Star_nose_sensor/Capacitance_measurement/logs/ramp_collector_20260714_163437_flat_0.csv'
    output_dir = 'Star_nose_sensor/Capacitance_measurement/data_analysis_ws_flat/plots_0mm'
    os.makedirs(output_dir, exist_ok=True)

    df = pd.read_csv(csv_path)

    # Read the actual depth_mm straight out of the file instead of guessing it --
    # a hardcoded value that doesn't match what's really in the CSV is exactly what
    # produced the "Number of columns must be a positive integer, not 0" crash
    # (self.rounds ends up empty, so n = len(self.rounds) = 0).
    found_depths = df['depth_mm'].unique()
    if len(found_depths) != 1:
        raise ValueError(
            f"{csv_path} contains more than one depth_mm value: {sorted(found_depths.tolist())}. "
            f"Expected exactly one depth per file -- pass the specific depth_mm you want instead."
        )
    depth = found_depths[0]
